...
/* 이층은 커널 크기가 3픽셀인 16개 필터를 사용하고 렐루 활성화 함수를 사용함 */
function createConvModel() {
  ...
  /* 첫 번째중 다음에 최대 플링층을 둔다 */
  // 주어진 영역에서 최댓값을 고르며 다운 샘플링처럼 동작한다.
  model.add(tf.layers.maxPooling2d({ poolSize: 2, strides: 2 }));

  // 세번째 층을 다른 합성곱 층, 이번에는 32개의 필터를 사용
  model.add(
  ...
  // 세번째 conv 추거
  model.add(
    tf.layers.conv2d({ kernelSize: 2, filters: 32, activation: "relu" })
  );

  // 마지막층에 입력으로 전달하기 위해 3D 텐서를 1D 백터로 펼침
  // 마지막 분류층에 고차원 데이터를 전달하기 위해 자주 사용하는 기법
